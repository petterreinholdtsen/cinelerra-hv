 ...
 \
	...
	{ \
		...
		case TRANSFER_REPLACE: \
			...
			break; \
		case TRANSFER_NORMAL: \
			r = ((int64_t)input1 * pixel_opacity + \
				(int64_t)output[0] * pixel_transparency) / max / max; \
			g = ((int64_t)(input2 - chroma_offset) * pixel_opacity + \
				(int64_t)(output[1] - chroma_offset) * pixel_transparency) \
				/ max / max + \
				chroma_offset; \
			b = ((int64_t)(input3 - chroma_offset) * pixel_opacity + \
				(int64_t)(output[2] - chroma_offset) * pixel_transparency) \
				/ max / max + \
				chroma_offset; \
			a = input4 > output[3] ? input4 : output[3]; \
			break; \
