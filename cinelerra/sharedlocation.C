...
int SharedPluginLocation::save(FileXML *xml)
{
	xml->tag.set_title("SHARED");
	xml->tag.set_property("MODULE", (long)module);
	xml->tag.set_property("PLUGIN", (long)plugin);
	xml->append_tag();
}

int SharedPluginLocation::load(FileXML *xml, int track_offset)
{
	module = xml->tag.get_property("MODULE", (long)0) + track_offset;
	plugin = xml->tag.get_property("PLUGIN", (long)0);
}
...
int SharedModuleLocation::save(FileXML *xml)
{
...
int SharedModuleLocation::load(FileXML *xml, int track_offset)
{
	module = xml->tag.get_property("MODULE", (long)0) + track_offset;
}
