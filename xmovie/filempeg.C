...
// Copy tables if a file is already opened by MWindow
	...
	mpeg3_set_mmx(fd, FileBase::file->use_mmx);
	read_header();
...
int FileMPEG::get_position(double &percentage, double &seconds)
{
	if(fd)
	{
		if(asset->video_data)
		{
			percentage = mpeg3_tell_percentage(fd);
...
// Audio only
		...
		if(asset->audio_data)
		{
			percentage = (double)mpeg3_get_sample(fd, 0) / mpeg3_audio_samples(fd, 0);
			seconds = (double)mpeg3_get_sample(fd, 0) / asset->rate;
		}
	}
	else
	{
...
int FileMPEG::set_position(double percentage)
{
	...
	if(fd)
	{
//printf("FileMPEG::set_position %f\n", percentage);
		if(asset->video_data)
			result = mpeg3_seek_percentage(fd, percentage);
 		else
 		if(asset->audio_data)
 		{
