 ...
 : Thread(1, 0, 0)
{
	...
	audio = 0;
	video = 0;
	config = new PlaybackConfig(PLAYBACK_LOCALHOST, 0);
	arender = 0;
	vrender = 0;
	do_audio = 0;
	do_video = 0;
...
// Retool configuration for this node
	...
	*this->config = *command->get_edl()->session->get_playback_config(playback_strategy, head_number);
