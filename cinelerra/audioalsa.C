...
void AudioALSA::list_devices(ArrayList<char*> *devices, int pcm_title)
{
	...
	{
		printf("AudioALSA::list_devices: no soundcards found\n");
		return;
	}
	...
	while(card >= 0) 
	{
		...
		while(1)
		{
			...
			{
				sprintf(string, "%s #%d", snd_ctl_card_info_get_name(info), dev);
			}
...
//printf("AudioALSA::set_params %d %d\n", samples,  device->device_buffer);
