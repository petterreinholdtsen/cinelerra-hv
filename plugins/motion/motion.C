...
int MotionWindow::create_objects()
{
	...
	add_subwindow(canvas = new MotionCanvas(plugin, 
		x, 
		y, 
		get_w() - 20, 
		get_h() - 140));
	...
	y += 30;
	x = 10;
	add_subwindow(new MotionReset(plugin, x, y));
	x += 100;
	add_subwindow(mode_motion = new MotionMode(plugin, 
		x, 
		y, 
		MotionConfig::MOTION,
		"Motion"));
	x += 120;
	add_subwindow(mode_sheer = new MotionMode(plugin, 
		x, 
		y, 
		MotionConfig::SHEER,
		"Sheer"));
	x = 110;
	y += 30;
	add_subwindow(mode_stretch = new MotionMode(plugin, 
		x, 
		y, 
		MotionConfig::STRETCH,
		"Stretch"));
	update_canvas();
	y += 30;
	x = 10;
	add_subwindow(new BC_Title(x, y, "Motion direction:"));
	x += 170;
	add_subwindow(forward = new MotionDirection(plugin, 
		x, 
		y, 
		1,
		"Forward"));
	x += 100;
	add_subwindow(reverse = new MotionDirection(plugin, 
		x, 
		y, 
		0,
		"Reverse"));
...
int MotionWindow::resize_event(int w, int h)
{
	return 1;
}

void MotionWindow::update_canvas()
{
	canvas->clear_box(0, 0, canvas->get_w(), canvas->get_h());
	int x1, y1, x2, y2, x3, y3, x4, y4;
	calculate_canvas_coords(x1, y1, x2, y2, x3, y3, x4, y4);

// printf("MotionWindow::update_canvas %d,%d %d,%d %d,%d %d,%d\n",
// x1,
// y1,
// x2,
// y2,
...
{
	mode_motion->update(plugin->config.mode == MotionConfig::MOTION);
	mode_sheer->update(plugin->config.mode == MotionConfig::SHEER);
	mode_stretch->update(plugin->config.mode == MotionConfig::STRETCH);
	forward->update(plugin->config.forward);
	reverse->update(!plugin->config.forward);
}

void MotionWindow::update_coord()
{
	x->update(plugin->get_current_x());
	y->update(plugin->get_current_y());
}

void MotionWindow::calculate_canvas_coords(int &x1, 
	int &y1, 
	int &x2, 
	int &y2, 
	int &x3, 
	...
	int &y4)
{
	...
	if(plugin->config.mode == MotionConfig::MOTION ||
		plugin->config.mode == MotionConfig::STRETCH)
	{
...
MotionCanvas::MotionCanvas(MotionMain *plugin, 
	int x, 
	int y, 
	int w,
	int h)
...
int MotionCanvas::button_press_event()
{
...
// printf("MotionCanvas::button_press_event %f %d %d %d %d\n", 
// distance3,
// cursor_x,
// cursor_y,
// x3,
// y3);
		...
		if(plugin->config.mode == MotionConfig::SHEER)
		{
			if(plugin->config.current_point == 1)
				plugin->config.current_point = 0;
			else
		...
		{
			if(alt_down())
				state = MotionCanvas::DRAG_FULL;
			else
				state = MotionCanvas::ZOOM;
...
int MotionCanvas::button_release_event()
{
	if(state != MotionCanvas::NONE)
	{
		state = MotionCanvas::NONE;
		return 1;
	}
...
int MotionCanvas::cursor_motion_event()
{
	if(state != MotionCanvas::NONE)
	{
		int w = get_w() - 1;
		int h = get_h() - 1;
		if(state == MotionCanvas::DRAG)
		{
		...
		if(state == MotionCanvas::DRAG_FULL)
		{
...
MotionCoord::MotionCoord(MotionWindow *gui,
	MotionMain *plugin, 
	int x, 
	int y,
	float value,
...
MotionReset::MotionReset(MotionMain *plugin, 
	int x, 
	int y)
...
MotionMode::MotionMode(MotionMain *plugin, 
	int x, 
	int y,
	int value,
	char *text)
...
MotionDirection::MotionDirection(MotionMain *plugin, 
	int x, 
	int y,
	int value,
	char *text)
...
int MotionDirection::handle_event()
{
...
void MotionMain::update_gui()
{
	if(thread)
	{
//printf("MotionMain::update_gui 1\n");
		thread->window->lock_window();
//printf("MotionMain::update_gui 2\n");
		load_configuration();
		thread->window->update_coord();
		thread->window->update_mode();
		thread->window->update_canvas();
		thread->window->unlock_window();
//printf("MotionMain::update_gui 3\n");
	}
}
...
float MotionMain::get_current_x()
{
...
float MotionMain::get_current_y()
{
	switch(config.current_point)
	{
		case 0:
...
void MotionMain::set_current_x(float value)
{
	switch(config.current_point)
	{
		case 0:
...
void MotionMain::set_current_y(float value)
{
	switch(config.current_point)
	{
		case 0:
...
int MotionMain::process_realtime(VFrame *input_ptr, VFrame *output_ptr)
{
	...
	if(temp && 
		config.mode == MotionConfig::STRETCH &&
	...
	if(temp &&
		config.mode == MotionConfig::MOTION &&
		(temp->get_w() != w ||
			temp->get_h() != h))
	{
		delete temp;
	...
	if(config.mode == MotionConfig::STRETCH)
	{
		if(!temp)
		{
			temp = new VFrame(0,
	...
	if(config.mode == MotionConfig::MOTION ||
		config.mode == MotionConfig::SHEER)
	{
		if(input_ptr->get_rows()[0] == output_ptr->get_rows()[0])
		{
			if(!temp) 
...
//printf("MotionMain::process_realtime %p %p %p\n", input, temp, output);
...
void MotionMatrix::translate(double x, double y)
{
	double g = values[2][0];
	double h = values[2][1];
	double i = values[2][2];
...
void MotionMatrix::scale(double x, double y)
{
	values[0][0] *= x;
	values[0][1] *= x;
	values[0][2] *= x;
...
void MotionMatrix::multiply(MotionMatrix *dst)
{
	int i, j;
	MotionMatrix tmp;
	double t1, t2, t3;
...
double MotionMatrix::determinant()
{
	double determinant;
...
void MotionMatrix::invert(MotionMatrix *dst)
{
	double det_1;
...
void MotionMatrix::copy_from(MotionMatrix *src)
{
	memcpy(&values[0][0], &src->values[0][0], sizeof(values));
}

void MotionMatrix::transform_point(float x, 
	float y, 
	float *newx, 
	float *newy)
{
...
void MotionMatrix::dump()
{
	printf("MotionMatrix::dump\n");
...
void MotionUnit::calculate_matrix(
	double in_x1,
	double in_y1,
	double in_x2,
	double in_y2,
...
/* Determine the motion transform that maps from
...
// printf("MotionUnit::calculate_matrix %f %f %f %f %f %f\n",
// dx1,
// dx2,
// dx3,
// dy1,
...
// printf("MotionUnit::calculate_matrix 1 %f %f\n", dx3, dy3);
// matrix.dump();
...
// printf("MotionUnit::calculate_matrix 4 %p\n", result);
// result->dump();
...
// Calculate real coords
	...
	if(plugin->config.mode == MotionConfig::STRETCH ||
		plugin->config.mode == MotionConfig::MOTION)
	{
	...
	if(plugin->config.mode == MotionConfig::MOTION ||
		plugin->config.mode == MotionConfig::SHEER)
	{
		MotionMatrix matrix;
		float temp;
		temp = out_x4;
...
// printf("MotionUnit::process_package 1 %f %f %f %f %f %f %f %f\n", 
// out_x1,
// out_y1,
// out_x2,
// out_y2,
...
#define CUBIC_ROW(in_row, chroma_offset) \
	transform_cubic(dx, \
		...
		yinc = m.values[1][0];
		winc = m.values[2][0];
//printf("MotionUnit::process_package 1 %d %d %d %d %f %f\n", tx1, ty1, tx2, ty2, out_x4, out_y4);
...
{ \
 ...
 \
	...
	{ \
		...
		{
			case BC_RGB888:
				MOTION(unsigned char, 3)
				break;
			case BC_RGBA8888:
				MOTION(unsigned char, 4)
				break;
			case BC_YUV888:
				MOTION(unsigned char, 3)
				break;
...
void MotionEngine::init_packages()
{
	...
	{
		MotionPackage *package = (MotionPackage*)packages[i];
		package->y1 = y1;
