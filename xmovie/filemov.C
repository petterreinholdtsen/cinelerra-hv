...
//printf("FileMOV::read_frame %d\n", quicktime_video_position(fd, file->video_stream));
// printf("FileMOV::read_frame 1 %p %d %d %d\n", 
// 	frame, 
// 	in_y1, 
// 	in_y2, 
		...
		switch(frame->get_color_model())
		{
			...
			case BC_YUV422P:
			{
				...
				result = quicktime_decode_scaled(fd, 
					...
					frame->get_h(),
					frame->get_color_model(),             /* One of the color models defined above */
					temp_rows, 
					file->video_stream);
				break;
			}
...
//for(int i = 0; i < 500; i++) frame->get_data()[i] = 255;
